  ...
  $.fn.extend({
    wodry: function(config) {
      ...
      return this.map(function() {
        ...
        flip_container = $(this);
        array = [];
        $.each(flip_container.text().split(settings.separator), function(
          key,
          value
        ) {
        ...
        if (settings.styles.length > 0) {
          flip_container.html(
            "<span class='" + settings.styles[0] + "'>" + array[0] + "</span>"
          );
        } else {
        ...
        animate = function(animation, container, currentText, nextText) {
          ...
          $("." + container[0].className + " .front-face").css(
            prefixer(["transform"], [animation.front_transform])
          );
          $(
            "<span class='" + back_style + "'>" + nextText + "</span>"
          ).appendTo(container);
          $("." + container[0].className + " .back-face").css(
            prefixer(["transform"], [animation.back_transform])
          );
          container
            ...
            .css(
              prefixer(
                ...
                [animation.action.transform, animation.action.transition]
              )
            );
          if (animation.isCoplex) {
            return setTimeout(function() {
              return $("." + container[0].className + " .front-face").remove();
            }, 1);
          }
        };
        flip = function() {
